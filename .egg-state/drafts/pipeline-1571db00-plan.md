...
**Risks / edge cases**: No existing code to break. For the algorithm: handle empty input, single interval, adjacent non-overlapping intervals, unsorted input, duplicates, and single-point intervals.
...
**Tasks**:
1. **[TASK-1-1]** Configure Python project: update `pyproject.toml` with ruff and pytest settings (line-length=88, target Python 3.12, src layout for pytest), add dev dependencies via `uv add --dev ruff pytest`, set `.python-version` to 3.12. Acceptance: `uv run ruff --version` and `uv run pytest --version` work.
...
phases:
  - id: 1
    ...
    tasks:
      - id: TASK-1-1
        description: "Configure Python project: update `pyproject.toml` with ruff and pytest settings (line-length=88, target py312, src layout for pytest), add dev dependencies via `uv add --dev ruff pytest`, set `.python-version` to 3.12"
