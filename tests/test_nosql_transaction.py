...
def populated_db(db: Database) -> Database:
    ...
    users.insert_one({"_id": "u1", "name": "Alice", "age": 30})
    users.insert_one({"_id": "u2", "name": "Bob", "age": 25})
    users.insert_one({"_id": "u3", "name": "Charlie", "age": 35})
    orders.insert_one({"_id": "o1", "user": "u1", "total": 100})
    return db
...
class TestBasicTransaction:
    ...
    def test_insert_then_commit(self, db: Database) -> None:
        ...
        col = txn.collection("users")
        col.insert_one({"_id": "u1", "name": "Alice", "age": 30})
        txn.commit()

        result = db.collection("users").find({"_id": "u1"})
        assert len(result) == 1
    ...
    def test_update_then_commit(self, populated_db: Database) -> None:
        ...
        result = populated_db.collection("users").find({"_id": "u1"})
    ...
    def test_update_then_rollback(self, populated_db: Database) -> None:
        ...
        txn = populated_db.begin_transaction()
        col = txn.collection("users")
        col.update_one({"_id": "u1"}, {"$set": {"age": 99}})
        txn.rollback()
...
class TestSnapshotIsolation:
    ...
    def test_reads_see_snapshot_at_start(self, populated_db: Database) -> None:
        ...
        result = txn.collection("users").find({"_id": "u1"})
        assert len(result) == 1
        assert result[0]["name"] == "Alice"
    ...
    def test_external_changes_not_visible_inside(self, populated_db: Database) -> None:
        ...
        populated_db.collection("users").update_one(
            {"_id": "u1"}, {"$set": {"name": "Alicia"}}
        )
    ...
    def test_own_writes_visible_within_transaction(
        ...
        txn = populated_db.begin_transaction()
        txn.collection("users").update_one({"_id": "u1"}, {"$set": {"age": 50}})
    ...
    def test_committed_changes_visible_outside(self, populated_db: Database) -> None:
        ...
        txn = populated_db.begin_transaction()
        txn.collection("users").update_one({"_id": "u2"}, {"$set": {"name": "Bobby"}})
        txn.commit()

        result = populated_db.collection("users").find({"_id": "u2"})
    ...
    def test_external_insert_not_visible_inside(self, populated_db: Database) -> None:
        ...
        populated_db.collection("users").insert_one(
            {"_id": "u4", "name": "Diana", "age": 28}
        )

        result = txn.collection("users").find({"_id": "u4"})
        assert len(result) == 0
    ...
    def test_external_delete_not_visible_inside(self, populated_db: Database) -> None:
        ...
        populated_db.collection("users").delete_one({"_id": "u2"})

        result = txn.collection("users").find({"_id": "u2"})
        assert len(result) == 1
...
class TestAtomicity:
    ...
    def test_multiple_ops_all_commit(self, db: Database) -> None:
        ...
        txn = db.begin_transaction()
        txn.collection("users").insert_one({"_id": "u1", "name": "Alice", "age": 30})
        txn.collection("users").insert_one({"_id": "u2", "name": "Bob", "age": 25})
        txn.collection("orders").insert_one({"_id": "o1", "user": "u1", "total": 50})
        txn.commit()

        assert len(db.collection("users").find({})) == 2
        assert len(db.collection("orders").find({})) == 1
    ...
    def test_rollback_undoes_all_operations(self, db: Database) -> None:
        ...
        txn.collection("users").insert_one({"_id": "u1", "name": "Alice", "age": 30})
        txn.collection("users").insert_one({"_id": "u2", "name": "Bob", "age": 25})
        txn.collection("users").insert_one({"_id": "u3", "name": "Charlie", "age": 35})
        txn.rollback()

        assert len(db.collection("users").find({})) == 0
    ...
    def test_mixed_insert_update_delete(self, populated_db: Database) -> None:
        ...
        txn = populated_db.begin_transaction()
        txn.collection("users").insert_one({"_id": "u4", "name": "Diana", "age": 28})
        txn.collection("users").update_one({"_id": "u1"}, {"$set": {"age": 31}})
        txn.collection("users").delete_one({"_id": "u3"})
        txn.commit()

        users = populated_db.collection("users")
        assert len(users.find({"_id": "u4"})) == 1
        assert users.find({"_id": "u1"})[0]["age"] == 31
        assert len(users.find({"_id": "u3"})) == 0
        # u2 unchanged
        assert users.find({"_id": "u2"})[0]["name"] == "Bob"

    def test_mixed_ops_rollback(self, populated_db: Database) -> None:
        """All mixed operations are undone on rollback."""
        txn = populated_db.begin_transaction()
        txn.collection("users").insert_one({"_id": "u4", "name": "Diana", "age": 28})
        txn.collection("users").update_one({"_id": "u1"}, {"$set": {"age": 99}})
        txn.collection("users").delete_one({"_id": "u2"})
        txn.rollback()

        users = populated_db.collection("users")
        assert len(users.find({"_id": "u4"})) == 0
        assert users.find({"_id": "u1"})[0]["age"] == 30
        assert len(users.find({"_id": "u2"})) == 1
...
class TestConflictDetection:
    ...
    def test_concurrent_modify_same_doc_conflict(self, populated_db: Database) -> None:
        ...
        txn1.collection("users").update_one({"_id": "u1"}, {"$set": {"age": 31}})
        txn2.collection("users").update_one({"_id": "u1"}, {"$set": {"age": 32}})
    ...
    def test_concurrent_modify_different_docs_no_conflict(
        ...
        txn1.collection("users").update_one({"_id": "u1"}, {"$set": {"age": 31}})
        txn2.collection("users").update_one({"_id": "u2"}, {"$set": {"age": 26}})
        ...
        users = populated_db.collection("users")
        assert users.find({"_id": "u1"})[0]["age"] == 31
        assert users.find({"_id": "u2"})[0]["age"] == 26
    ...
    def test_modify_doc_deleted_by_another_txn(self, populated_db: Database) -> None:
        ...
        txn1.collection("users").delete_one({"_id": "u1"})
        txn2.collection("users").update_one({"_id": "u1"}, {"$set": {"age": 50}})
    ...
    def test_both_insert_same_id_conflict(self, db: Database) -> None:
        ...
        txn1.collection("users").insert_one({"_id": "u1", "name": "Alice"})
        txn2.collection("users").insert_one({"_id": "u1", "name": "Bob"})
    ...
    def test_concurrent_modify_different_collections(
        ...
        txn1.collection("users").update_one({"_id": "u1"}, {"$set": {"age": 31}})
        txn2.collection("orders").update_one({"_id": "o1"}, {"$set": {"total": 200}})

        txn1.commit()
        txn2.commit()

        assert populated_db.collection("users").find({"_id": "u1"})[0]["age"] == 31
        assert populated_db.collection("orders").find({"_id": "o1"})[0]["total"] == 200
...
class TestTransactionEdgeCases:
    ...
    def test_transaction_on_empty_collection(self, db: Database) -> None:
        ...
        txn = db.begin_transaction()
        result = txn.collection("nonexistent").find({})
        assert len(result) == 0
        txn.rollback()
    ...
    def test_multiple_sequential_transactions(self, db: Database) -> None:
        ...
        txn2 = db.begin_transaction()
        txn2.collection("users").insert_one({"_id": "u2", "name": "Bob"})
        txn2.commit()

        txn3 = db.begin_transaction()
        result = txn3.collection("users").find({})
        assert len(result) == 2
        txn3.rollback()
    ...
    def test_nested_collection_ops_in_transaction(self, db: Database) -> None:
        ...
        txn = db.begin_transaction()
        txn.collection("users").insert_one({"_id": "u1", "name": "Alice"})
        txn.collection("orders").insert_one({"_id": "o1", "user": "u1", "total": 50})
        txn.collection("products").insert_one(
            {"_id": "p1", "name": "Widget", "price": 10}
        )
        txn.commit()

        assert len(db.collection("users").find({})) == 1
        assert len(db.collection("orders").find({})) == 1
        assert len(db.collection("products").find({})) == 1

    def test_large_number_of_operations(self, db: Database) -> None:
        """Transaction handles many operations without issue."""
        txn = db.begin_transaction()
        col = txn.collection("items")
        for i in range(200):
            col.insert_one({"_id": f"item_{i}", "value": i})
        txn.commit()

        result = db.collection("items").find({})
    ...
    def test_commit_then_new_transaction_sees_changes(self, db: Database) -> None:
        ...
        txn1 = db.begin_transaction()
        txn1.collection("users").insert_one({"_id": "u1", "name": "Alice", "age": 30})
        txn1.commit()

        txn2 = db.begin_transaction()
        txn2.collection("users").update_one({"_id": "u1"}, {"$set": {"age": 31}})
        txn2.commit()
    ...
    def test_transaction_delete_hides_in_same_txn(self, populated_db: Database) -> None:
        ...
        txn = populated_db.begin_transaction()
        txn.collection("users").delete_one({"_id": "u1"})
